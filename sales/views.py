...
def sale_create(request):
    ...
    return render(request, "sales/form.html", {"form": form, "title": "Nouvelle Vente"})
...
def sale_detail(request, pk):
    ...
    if hasattr(sale.vehicle.vehicle_purchase, "freight_cost"):
        cost_breakdown["freight_cost"] = (
            sale.vehicle.vehicle_purchase.freight_cost.total_freight_cost_da or 0
        )

    if hasattr(sale.vehicle.vehicle_purchase, "customs_declaration"):
        cost_breakdown["customs_cost"] = (
            sale.vehicle.vehicle_purchase.customs_declaration.total_customs_cost_da or 0
        )
...
def sale_delete(request, pk):
    """Delete a sale (manager only, non-finalized)"""
...
def sale_edit(request, pk):
    ...
    return render(
        request,
        "sales/form.html",
        {"form": form, "sale": sale, "title": f"Modifier Vente {sale.sale_number}"},
    )
...
def invoice_detail(request, pk):
    ...
    invoice = get_object_or_404(
        Invoice.objects.select_related(
            "sale__vehicle", "sale__assigned_trader", "customer"
        ),
        pk=pk,
    )
...
def invoice_print(request, pk):
    ...
    context = {
        "invoice": invoice,
        "config": SystemConfiguration.get_current(),  # ← add this
    }
...
def ajax_vehicle_details(request):
    ...
    try:
        vehicle = Vehicle.objects.select_related("vehicle_purchase__supplier").get(
            pk=vehicle_id
        )
...
def ajax_calculate_margin(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            margin_percentage = (
                (margin_amount / landed_cost * 100) if landed_cost > 0 else 0
            )
